...
export default function NavMobile(props: NavProps) {
  ...
  return (
    <nav className={clsx("relative h-full flex justify-between items-center", "lg:hidden")}>
      <Logo />
      <div
        className={clsx("fixed inset-0 pt-20 bg-background transition-transform translate-x-full", "dark:bg-zinc-900", {
          "translate-x-0": open,
        })}
      >
      ...
      <button onClick={toggleHamburgur} className="relative w-5 h-5">
        {open ? (
          <CloseIcon />
        ) : (
