...
const FacebookIcon = () => (
  <a
    ...
    target="_blank"
    rel="noreferrer"
    className={clsx("fill-emerald-500", "sm:hover:fill-background")}
  >
...
function BelowSmBottomBar({ liProps }: { liProps: any }) {
  return (
    <aside className={clsx("fixed inset-0 z-10 top-auto h-14 bg-transparent", "sm:hidden", "")}>
...
function AboveSmSideBar({ liProps }: { liProps: any }) {
  return (
    <aside className={clsx("hidden fixed bottom-0 left-20", "sm:block sm:left-5 lg:left-14 xl:left-30 2xl:left-40")}>
      <div className="flex flex-col items-center">
        ...
        <div className="w-1 h-32 bg-zinc-200"></div>
      </div>
    </aside>
  );
}
...
export default function SideBar() {
  ...
  return (
    <>
      <BelowSmBottomBar liProps={liProps} />
      <AboveSmSideBar liProps={liProps} />
    </>
  );
}
